...
impl PlonkVerifier {
    pub fn verify(pre_in: &PreprocessedInput, proof: &PlonkProof, transcript: &mut Transcript) -> Result<(), ()> {
        ...
        transcript.append_point(b"commitment a", &proof.a.0);
        transcript.append_point(b"commitment b", &proof.b.0);
        transcript.append_point(b"commitment c", &proof.c.0);

        let beta = transcript.challenge_scalar(b"beta");
        let gamma = transcript.challenge_scalar(b"gamma");

        transcript.append_point(b"Permutation polynomial", &proof.z.0);
        ...
        let batch_poly_commit_1 = proof.a_eval * proof.b_eval * qm_comm + proof.a_eval * ql_comm + proof.b_eval * qr_comm + proof.c_eval * qo_comm + qc_comm +
            ((proof.a_eval + beta * zeta + gamma) * (proof.b_eval + beta * K1() * zeta + gamma) * (proof.c_eval + beta * K2() * zeta + gamma) * alpha + pre_in.constraints.lagrange_basis(0).eval(&zeta) * alpha * alpha + u) * &proof.z +
